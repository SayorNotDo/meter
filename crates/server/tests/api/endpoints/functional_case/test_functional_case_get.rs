...
use server::dto::{
    ...
    response::{case::FunctionalCaseResponse, ListFunctionalCaseResponse},
};
...
pub async fn test_success_get_functional_case(ctx: &mut SeedDbTestContext) {
    ...
    assert!(matches!(
        resp,
        AppResponseResult::Ok(ListFunctionalCaseResponse { .. })
    ));
    if let AppResponseResult::Ok(case_list_res) = resp {
        for item in case_list_res.list.into_iter() {
            let (status, resp) = ctx
                .app
                .api
                .get_functional_case(&token.access_token, ctx.project.id, item.id)
                .await
                .unwrap();
            assert!(status.is_success(), "status: {status}");
            assert!(matches!(
